...
dusche <- full_cntry_list %>%
  split(1:nrow(.)) %>% 
  map(~{
    .x$iso2c %>% paste0(c("-last_7_days", "-last_30_days", "-last_90_days")) %>% 
      set_names(.x$country)
  }) %>% unlist()
...
releases <- pb_releases()$release_name

dusche %>% tibble::enframe() %>% mutate(name = str_remove(name, ".*\\.")) %>% 
  separate(value, remove = F, into = c("cntry", "tframe"), sep = "-") %>% 
  filter(!(value %in% releases)) %>% 
  split(1:nrow(.)) %>% 
  walk_progress(~{
    the_tag <- .x$value
    # if(!(the_tag %in% releases)){
      pb_release_create(repo = "favstats/meta_ad_reports", tag = the_tag, body = paste0(paste0("This release includes ", .x$name ," '", .x$tframe ,"' Meta ad spending reports.")))
      # Sys.sleep(5)
    # }
  })
...
report_paths <- dir(paste0("report"), full.names = T, recursive = T) %>%
  ...
  keep(~str_detect(.x, "last_30_days"))
# .[200:202]
...
report_paths %>% 
  ...
  walk_progress(~{
    ...
    # tframe <- str_extract(extracted_path, "yesterday|last_90_days|last_30_days|last_90_days|lifelong")
    ...
    dir.create(paste0("storage30/",cntry_str, "/"), recursive = T)
    
    thedata %>%
      readr::write_rds(paste0("storage30/",cntry_str, "/", the_date, ".rds"), compress = "xz")
    ...
    file.copy(paste0("storage30/",cntry_str, "/", the_date, ".rds"), paste0("storage30/",cntry_str, "/latest.rds"))
    file.copy(report_path, paste0("storage30/",cntry_str, "/", the_date, ".zip"), overwrite = T)
