...
dusche <- full_cntry_list$iso2c %>% 
  map(~{
    .x %>% paste0(c("-last_7_days", "-last_30_days", "-last_90_days")) 
  }) %>% unlist()
...
report_paths <- dir(paste0("report"), full.names = T, recursive = T) %>%
  ...
  keep(~str_detect(.x, "last_90_days"))
# .[200:202]
...
report_paths %>% 
  ...
  walk_progress(~{
    ...
    dir.create(paste0("storage90/",cntry_str, "/"), recursive = T)
    
    thedata %>%
      readr::write_rds(paste0("storage90/",cntry_str, "/", the_date, ".rds"), compress = "xz")
    ...
    file.copy(paste0("storage90/",cntry_str, "/", the_date, ".rds"), paste0("storage90/",cntry_str, "/latest.rds"))
    file.copy(report_path, paste0("storage90/",cntry_str, "/", the_date, ".zip"), overwrite = T)
...
#     tframe <- str_extract(.x, "yesterday|last_90_days|last_30_days|last_90_days|lifelong")
#     
#     
#     
